...
public class InitialState: HistoryItem
{
    ...
    required public init?(data: MessagePackValue)
    {
        ...
        guard array.count >= InitialState.initialStateMaxKey+1 else {
            ErrorHandling.error("InitialState data must have an " +
                                "array of size \(InitialState.initialStateMaxKey+1)!")
            return nil
        }
        ...
        guard packedPlayers.count > 0 else {
            ErrorHandling.error("There will always be at least two players " +
                                "even if they have no dice, in an InitialState")
            return nil
        }

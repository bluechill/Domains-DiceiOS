...
public class ChallengeAction: HistoryAction
{
    ...
    static private let IndexKey: Int = HistoryAction.actionMaxKey+2
    ...
    required public init?(data: MessagePackValue)
    {
        ...
        guard array.count >= ChallengeAction.challengeMaxKey+1 else {
            ErrorHandling.error("ChallengeAction data must have " +
                                "\(ChallengeAction.challengeMaxKey+1) items")
            return nil
        }
        ...
        guard let challengeActionIndex = array[ChallengeAction.IndexKey].integerValue else {
